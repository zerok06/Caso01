# Compilación de Prompts del Sistema

Este archivo contiene una lista detallada de los prompts utilizados en el sistema, excluyendo el "Checklist Analyzer". Se incluye la ubicación, el propósito y el texto crudo de cada prompt.

## 1. System Prompt (Base RAG)
**Ubicación:** `backend/core/providers/llm_provider.py` (Método `_build_prompt`)
**Propósito:** Prompt base para el sistema RAG. Define la personalidad del asistente y las reglas estrictas para responder basándose únicamente en el contexto recuperado.
...
## 2. Intent Detector (Clasificador de Intención)
**Ubicación:** `backend/core/intent_detector.py`
**Propósito:** Clasificar la entrada del usuario para enrutarla al flujo de trabajo adecuado (Generar propuesta, consulta general, matriz, etc.).
...
## 3. Análisis RFP - Extracción JSON
**Ubicación:** `backend/prompts/proposals/analyze_prompts.py` (`create_analysis_JSON_prompt`)
**Propósito:** Extraer datos estructurados clave de un documento RFP para procesarlos programáticamente.
...
## 4. Generación de Propuesta Markdown
**Ubicación:** `backend/prompts/proposals/analyze_prompts.py` (`create_markdown_analysis_prompt`)
**Propósito:** Generar el contenido completo de una propuesta técnica y comercial.
...
## 5. Tareas de Intención Específica
**Ubicación:** `backend/api/routes/intention_task.py`
**Propósito:** Prompts cortos y directos para funcionalidades de chat específicas.

### 5.1 Consulta General (`general_query_chat`)
```text
Responde de manera clara y concisa a las
preguntas basada en el contexto proporcionado.
```

### 5.2 Matriz de Requisitos (`requirements_matrix_chat`)
```text
OBJETIVO: Generar los requerimientos funcionales y no funcionales.
A TENER EN CUENTA: Analiza todo el documento RFP/RFI. Los requerimientos funcionales no son de proceso son del sistema, normalmente está especificado en alguna parte del documento .
IMPORTANTE: Devuelveme los requermientos funcionales y no funcionales tal cual está en el documento.
...
### 5.3 Cotización Preliminar (`preeliminar_price_quote_chat`)
```text
...
### 5.4 Riesgos Legales (`legal_risks_chat`)
```text
...
### 5.5 Consulta Específica (`specific_query_chat`)
```text
...
## 6. Generación de Documentos Descargables
**Ubicación:** `backend/api/routes/document_generation.py` (`_generate_document_content`)
**Propósito:** Sintetizar el historial del chat en un documento completo para descarga.
